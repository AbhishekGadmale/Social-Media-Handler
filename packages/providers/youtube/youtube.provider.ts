...
export class YouTubeProvider implements ISocialProvider, IPublishingProvider {
  ...
  getAuthorizationUrl(
    input: AuthUrlInput & { codeChallenge?: string },
  ): string {
    ...
    if (input.requestedScopes) {
      input.requestedScopes.forEach((s) => {
        if (!ALLOWED_SCOPES.includes(s)) {
          throw new Error(
            `Invalid or unauthorized OAuth scope requested: ${s}`,
          );
        }
        scopes.add(s);
      });
  ...
  async getProfiles(
    ...
    if (!response.ok) {
      const error = await response.text();
      throw new ProviderApiError(
        `Failed to fetch YouTube channels: ${error}`,
        response.status,
      );
    }
  ...
  async getAccountMetrics(
    ...
    try {
      ...
      if (!response.ok) {
        const error = await response.text();
        throw new ProviderApiError(
          `Failed to fetch YouTube channel statistics: ${error}`,
          response.status,
        );
      }
      ...
      if (error.name === "ProviderCoordinationError") {
        throw error;
      }
      if (error.name === "AbortError" || error.type === "aborted") {
        throw new Error(
          `YouTube API request timed out after 10s for channel ${account.id}`,
        );
      }
      throw error;
    } finally {
    ...
    try {
      ...
      if (errorName === "NoSuchKey" || errorName === "NotFound") {
        return {
          ...
          message:
            "The media file could not be found in storage. It may have been deleted or not uploaded correctly.",
        };
      }
    ...
    try {
      ...
      if (!context?.beforeFinalMutation) {
        throw new ProviderCoordinationError(
          "YouTube publishing strictly requires beforeFinalMutation coordination hook.",
        );
      }
      ...
      if (status === 401) {
        return {
          ...
          message:
            "Invalid or expired authorization. Please reconnect your account.",
        };
      }
      ...
      if (status === 403) {
        ...
        if (reason === "insufficientPermissions") {
          return {
            ...
            message:
              "Insufficient permissions. Please reconnect and grant YouTube publish access.",
          };
        }
        ...
        return {
          ...
          message:
            "Upload forbidden by YouTube (could be channel restriction or permission issue).",
        };
      }
      ...
      return {
        ...
        message:
          "Unknown outcome due to network or server error. Cannot safely retry without verification.",
      };
    }
  }
    ...
    try {
      ...
      if (!res.data.items || res.data.items.length === 0) {
        return {
          status: "FAILED",
          failureCategory: "PERMANENT",
          failureCode: "YOUTUBE_VIDEO_DELETED_OR_MISSING",
          message: "Video was not found on YouTube during status check.",
        };
      }

      const uploadStatus = res.data.items[0].status?.uploadStatus;
      ...
      if (
        ...
        return {
          ...
          message: `YouTube processing failed with status: ${uploadStatus}`,
        };
      }
      ...
      return {
        ...
        message: error.message || "Unknown error checking YouTube status.",
      };
    }
  }
}
