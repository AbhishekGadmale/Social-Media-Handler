...
vi.mock("image-size", async (importOriginal) => {
  ...
  describe("Instagram Reels Publishing", () => {
    ...
    it("E. onRemotePrepared failure prevents later final mutation", async () => {
      mockContext.onRemotePrepared.mockRejectedValue(new Error("DB Down"));
      global.fetch = vi.fn().mockResolvedValue({
        ok: true,
        json: async () => ({ id: "cont_42" }),
      });
    ...
    it("L. beforeFinalMutation failure: /media_publish = 0", async () => {
      mockContext.beforeFinalMutation.mockRejectedValue(new Error("CAS Stale"));
      global.fetch = vi.fn();
...
describe("Instagram Provider Publishing", () => {
  ...
  describe("Capabilities", () => {
    it("supports IMAGE_POST and VIDEO_POST", () => {
      const caps = provider.getPublishingCapabilities();
      expect(caps.contentTypes.IMAGE_POST.supported).toBe(true);
      expect(caps.contentTypes.IMAGE_POST.maxBytes).toBe(8 * 1024 * 1024);
