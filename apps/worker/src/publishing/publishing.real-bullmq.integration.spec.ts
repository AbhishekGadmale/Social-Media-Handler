...
describe('REAL REDIS/BULLMQ: Publishing Architecture Continuation', () => {
  ...
  beforeAll(async () => {
    module = await Test.createTestingModule({
      ...
      .useValue({
        getExecutionCredentials: jest.fn().mockResolvedValue({ accessToken: 'test' }),
      })
      .compile();
  ...
  async function createTestVariant(phase: string, nextCheckAtDelayMs = -1000, overrides: any = {}) {
    ...
    const account = await prisma.socialAccount.findFirst({ where: { workspaceId: wsId } });
  ...
  it('1. Proves worker does not sleep during PROCESSING_REMOTE & correctly schedules delayed continuation', async () => {
    ...
    const { variant } = await createTestVariant('PROCESSING_REMOTE', -1000, { remoteResourceId: 'yt-1' });
    ...
    const updatedVariant = await prisma.postPlatformVariant.findUniqueOrThrow({ where: { id: variant.id } });
    const meta = updatedVariant.executionMetadata as any;
    expect(meta.phase).toBe('PROCESSING_REMOTE');
    expect(new Date(meta.nextCheckAt).getTime()).toBeGreaterThan(Date.now() + 10000); // Definitely >10s in future
    
    // And if dispatcher scans again RIGHT NOW, it will NOT add a new job.
    await queue.drain();
    await dispatcher.scan();
    const jobs = await queue.getWaiting();
    expect(jobs.find(j => j.data.publicationId === variant.id)).toBeUndefined();
  });
  ...
  it('2. Deterministic Job ID / duplicate suppression', async () => {
    ...
    // Add twice rapidly
    mockAdapter.checkStatus.mockResolvedValueOnce({ processingState: 'PROCESSING' });
  ...
  it('3. Stale DispatchVersion test', async () => {
    ...
    const updated = await prisma.postPlatformVariant.findUniqueOrThrow({ where: { id: variant.id } });
  ...
  it('5. YouTube 3-poll checkStatus flow', async () => {
    ...
    const { variant, attempt } = await createTestVariant('PROCESSING_REMOTE', -1000, { remoteResourceId: 'yt-123' });
    const jobId = `publication-${variant.id}-v${variant.dispatchVersion}`;
    const payload = {
      workspaceId: variant.workspaceId,
      publicationId: variant.id,
    ...
    let updatedVariant = await prisma.postPlatformVariant.findUniqueOrThrow({ where: { id: variant.id } });
    ...
    updatedVariant = await prisma.postPlatformVariant.findUniqueOrThrow({ where: { id: variant.id } });
    payload.dispatchVersion = updatedVariant.dispatchVersion;
    ...
    const finalVariant = await prisma.postPlatformVariant.findUniqueOrThrow({ where: { id: variant.id } });
    expect(finalVariant.status).toBe('PUBLISHED');
    expect((finalVariant.executionMetadata as any).phase).toBe('COMPLETED');
    expect((finalVariant.executionMetadata as any).finalRemoteId).toBe('yt-123');

    // Ensure attempts count is exactly 1 (polling doesn't inflate rows)
    const attempts = await prisma.publicationAttempt.findMany({ where: { variantId: variant.id } });
  ...
  it('6. Instagram Reels 3-poll checkStatus flow & Finalize', async () => {
    ...
    const { variant, attempt } = await createTestVariant('PROCESSING_REMOTE', -1000, { remoteResourceId: 'ig-123' });
    const jobId = `publication-${variant.id}-v${variant.dispatchVersion}`;
    const payload = {
      workspaceId: variant.workspaceId,
      publicationId: variant.id,
    ...
    mockAdapter.finalizePublish.mockImplementationOnce(async (credentials, input, remoteId, ctx) => {
    ...
    expect(finalVariant.status).toBe('PUBLISHED');
    expect((finalVariant.executionMetadata as any).phase).toBe('COMPLETED');
    expect((finalVariant.executionMetadata as any).finalRemoteId).toBe('ig-pub-123');
  });

  it('7. Finalization CAS Failure (Worker Race Simulation BEFORE /media_publish)', async () => {
    // Reels uses READY -> finalizePublish
    const { variant } = await createTestVariant('PROCESSING_REMOTE', -1000, { remoteResourceId: 'ig-123' });
    const jobId = `publication-${variant.id}-v${variant.dispatchVersion}`;
    const payload = {
      workspaceId: variant.workspaceId,
      publicationId: variant.id,
    ...
    mockAdapter.finalizePublish.mockImplementationOnce(async (credentials, input, remoteId, ctx) => {
      ...
      const executionRepo = new ExecutionMetadataRepository(prisma, variant.workspaceId);
      ...
      await prisma.postPlatformVariant.update({
        where: { id: variant.id },
        data: { status: 'PUBLISHED', dispatchVersion: payload.dispatchVersion + 2 }
      });
  ...
  it('8. UNKNOWN FINAL MUTATION (media_publish timeout)', async () => {
    const { variant } = await createTestVariant('PROCESSING_REMOTE', -1000, { remoteResourceId: 'ig-8' });
    const jobId = `publication-${variant.id}-v${variant.dispatchVersion}`;
    const payload = { workspaceId: variant.workspaceId, publicationId: variant.id, dispatchVersion: variant.dispatchVersion, operationId: (variant.executionMetadata as any).operationId };

    mockAdapter.checkStatus.mockResolvedValueOnce({ status: 'READY' });
    mockAdapter.finalizePublish.mockImplementationOnce(async (c, i, r, ctx) => {
      await ctx.beforeFinalMutation();
  ...
  it('9. META PUBLISHED RECONCILIATION', async () => {
    const { variant } = await createTestVariant('PROCESSING_REMOTE', -1000, { remoteResourceId: 'ig-9' });
    const jobId = `publication-${variant.id}-v${variant.dispatchVersion}`;
    const payload = { workspaceId: variant.workspaceId, publicationId: variant.id, dispatchVersion: variant.dispatchVersion, operationId: (variant.executionMetadata as any).operationId };
  ...
  it('10. READ-ONLY STATUS TRANSPORT FAILURES', async () => {
    const { variant, attempt } = await createTestVariant('PROCESSING_REMOTE', -1000, { remoteResourceId: 'ig-10' });
    const jobId = `publication-${variant.id}-v${variant.dispatchVersion}`;
    const payload = { workspaceId: variant.workspaceId, publicationId: variant.id, dispatchVersion: variant.dispatchVersion, operationId: (variant.executionMetadata as any).operationId };
    ...
    let vA = await prisma.postPlatformVariant.findUniqueOrThrow({ where: { id: variant.id } });
    ...
    let vB = await prisma.postPlatformVariant.findUniqueOrThrow({ where: { id: variant.id } });
    expect(vB.status).toBe('PUBLISHING');
    expect((vB.executionMetadata as any).phase).toBe('PROCESSING_REMOTE');
    expect(new Date((vB.executionMetadata as any).nextCheckAt).getTime()).toBeGreaterThan(new Date((vA.executionMetadata as any).nextCheckAt).getTime());
  ...
  it('11. DEFINITIVE REMOTE FAILURE', async () => {
    const { variant } = await createTestVariant('PROCESSING_REMOTE', -1000, { remoteResourceId: 'ig-11' });
    const jobId = `publication-${variant.id}-v${variant.dispatchVersion}`;
    const payload = { workspaceId: variant.workspaceId, publicationId: variant.id, dispatchVersion: variant.dispatchVersion, operationId: (variant.executionMetadata as any).operationId };
    ...
    const vFail = await prisma.postPlatformVariant.findUniqueOrThrow({ where: { id: variant.id } });
  ...
  it('12. STALE handleUnknown TERMINAL PROTECTION', async () => {
    ...
    await prisma.postPlatformVariant.update({
      ...
      data: {
        ...
        executionMetadata: { ...(variant.executionMetadata as any), phase: 'COMPLETED', finalRemoteId: 'stale-test-12' }
      }
    });
    ...
    await queue.add('publish-job', payload, { jobId: `publication-${variant.id}-stale` });
    await sleep(300);

    // Assert protection
    const vEnd = await prisma.postPlatformVariant.findUniqueOrThrow({ where: { id: variant.id } });
  ...
  it('13. BULLMQ TECHNICAL RETRY', async () => {
    ...
    mockAdapter.finalizePublish.mockClear();
    mockAdapter.publish.mockClear();
    await queue.add('publish-job', payload, { jobId: `publication-${variant.id}-techretry` });
    await sleep(300);
